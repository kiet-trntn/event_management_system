...
function AddEvent() {
    ...
    const [formData, setFormData] = useState({ 
        ...
        end_date: '',
        max_members: '',
        leader_id: '' // Sẽ lưu ID nhưng người dùng sẽ chọn Tên
    });

    const [statusType, setStatusType] = useState('published');
    
    // 🌟 THÊM STATE ĐỂ LƯU DANH SÁCH NHÂN VIÊN
    const [users, setUsers] = useState([]);

    useEffect(() => {
        document.title = "Thêm sự kiện mới | TOOF";
        
        // 🌟 HÀM TỰ ĐỘNG LẤY DANH SÁCH NHÂN VIÊN ĐỂ ĐỔ VÀO DROPDOWN
        const fetchUsers = async () => {
            try {
                ...
                if (response.ok && Array.isArray(data)) {
                    // Lọc lấy những nhân viên đang active (nếu cần) hoặc lấy hết
                    setUsers(data);
    ...
    const handleSubmit = async (e) => {
        ...
        // Kiểm tra xem đã chọn Leader chưa
        ...
        try {
            const response = await fetch('http://localhost:5000/api/events', {
                ...
                body: JSON.stringify({ ...formData, status: statusType })
            });
            ...
            if (response.ok) {
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công!',
                    text: statusType === 'Nháp' ? 'Đã lưu bản nháp sự kiện.' : 'Đã đăng sự kiện mới (Sắp diễn ra).',
            ...
            <div className="form-card large">
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={{ display: 'flex', gap: '16px' }}>
                        ...
                        {/* 🌟 ĐÃ THAY ĐỔI: Biến ô nhập ID thành ô Chọn tên (Dropdown) */}
                        <div className="form-group" style={{ flex: 1 }}>
                    ...
                    <div className="form-actions">
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            ...
                            <button 
                                ...
                                onClick={() => setStatusType('Sắp diễn ra')}
                            >
                                Đăng sự kiện
                            </button>
                        </div>
